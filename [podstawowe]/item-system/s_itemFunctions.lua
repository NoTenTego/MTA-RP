    ...
    for i, item in pairs(items) do
        if tostring(item[5]) == itemDBID then
			...
			if delete or itemValue == 1 then --usuwa całkowicie przedmiot bez sprawdzenia jego value
				...
				mysql:execute(true, "DELETE FROM items WHERE id='"..itemDBID.."'")
			else
				item[3] = itemValue - 1
				setElementData(thePlayer, 'items', items)
			end
...
addEventHandler("item-system:useItem", root, function(name, value1, value2, itemID, itemDBID)
    itemID = tonumber(itemID)
    if itemID == 1 then
        if getElementData(client, "healthAddon") then
            return
        end
        triggerClientEvent(client, "item-system:closeEquipment", client)
        takeItem(client, itemDBID)
    ...
    elseif itemID == 2 then
        ...
        triggerClientEvent(client, 'hud:sendNotification', client, 'Ten przedmiot nic nie robi.')
    end
end)

addEvent("item-system:destroyItem", true)
addEventHandler("item-system:destroyItem", root, function(name, value1, value2, itemID, itemDBID)
    takeItem(client, itemDBID, true)
end)

function useFood(thePlayer)
	if getPedOccupiedVehicle(thePlayer) then
		triggerClientEvent(thePlayer, 'hud:sendNotification', thePlayer, "Nie możesz wykonać tej czynności będąc w pojeździe.")
		return
	end
...
function destroyFood(thePlayer)
	...
	if getElementData(thePlayer, "healthAddon") then
		...
		foodState[thePlayer] = 1
		triggerClientEvent(thePlayer, 'hud:sendNotification', thePlayer, "Wyrzucono jedzenie.")
	end
end